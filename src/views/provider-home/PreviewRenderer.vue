<template>
  <div class="preview-root">
    <div class="site-wrapper">
      ...
      <header class="hero" :style="heroStyle">
        <div class="hero-inner">
          <div class="hero-text">
            ...
            <p v-if="page.hero_description" class="lead">{{ page.hero_description }}</p>

            <div class="hero-cta" v-if="page.hero_cta_text || page.hero_cta_url">
              <a :href="page.hero_cta_url || '#'" class="btn-primary">
      ...
      <!-- BANNER SLIDER -->
      <section v-if="page.banner_slides && page.banner_slides.length" class="banner-slider">
        <div class="slides">
          <div v-for="slide in page.banner_slides" :key="slide.id" class="slide">
      ...
      <!-- FEATURES -->
      <section v-if="page.features && page.features.length" class="features-section">
        <div class="container">
          <h2 class="section-title">Features</h2>
          <div class="features-grid">
            <div v-for="f in page.features" :key="f.title" class="feature-card">
              <div class="feature-icon" v-if="f.icon">
                <i :class="f.icon"></i>
              </div>
              <h4>{{ f.title }}</h4>
              <p>{{ f.description }}</p>
      ...
      <!-- IMAGE GALLERY -->
      <section v-if="page.images && page.images.length" class="gallery-section">
        <div class="container">
          <h2 class="section-title">Gallery</h2>
          <div class="gallery-grid">
            <div v-for="img in page.images" :key="img.id" class="gallery-item">
              <img :src="img.image" :alt="img.caption || 'gallery'" loading="lazy" />
      ...
      <!-- TESTIMONIALS -->
      <section v-if="page.testimonials && page.testimonials.length" class="testimonials-section">
        <div class="container">
          <h2 class="section-title">What Customers Say</h2>
          <div class="testimonials-grid">
            <div v-for="t in page.testimonials" :key="t.id" class="testimonial-card">
              <div class="avatar" v-if="t.image">
                <img :src="t.image" :alt="t.name" loading="lazy" />
              </div>
              <div class="testimonial-body">
                <p class="review">“{{ t.review }}”</p>
                <p class="author">— {{ t.name }} <span v-if="t.role">, {{ t.role }}</span></p>
              </div>
            </div>
          </div>
        </div>
      ...
      <!-- VIDEOS -->
      <section v-if="page.videos && page.videos.length" class="videos-section">
        <div class="container">
          <h2 class="section-title">How it Works</h2>
          <div class="videos-grid">
            <div v-for="v in page.videos" :key="v.id" class="video-item">
              <video :src="v.url" controls :poster="v.thumbnail" preload="metadata" />
              <h4>{{ v.title }}</h4>
            </div>
          </div>
        </div>
      ...
      <!-- SERVICES -->
      <section v-if="page.services && page.services.length" class="services-section">
        <div class="container">
          <h2 class="section-title">Services</h2>
          <div class="services-grid">
            <div v-for="s in page.services" :key="s.id" class="service-card">
              <div class="service-icon" v-if="s.icon"><i :class="s.icon"></i></div>
              <h4>{{ s.title }}</h4>
              <p>{{ s.description }}</p>
              <a v-if="s.link" :href="s.link" class="link-more">Learn more</a>
            </div>
          </div>
        </div>
      </section>
      ...
      <!-- PLANS (from backend) -->
      <section v-if="page.plans && page.plans.length" class="plans-section">
        <div class="container">
          <h2 class="section-title">Plans</h2>
          <div class="plans-grid">
            <div v-for="p in page.plans" :key="p.id" class="plan-card">
              <h3>{{ p.title }}</h3>
              <p v-if="p.subtitle">{{ p.subtitle }}</p>

              <div class="plan-prices" v-if="p.prices && p.prices.length">
                <div v-for="price in p.prices" :key="price.id" class="price-row">
                  <strong>{{ price.amount }} {{ price.currency }}</strong>
                  <span class="cycle"> / {{ price.billing_cycle.name }}</span>
                </div>
              </div>

              <ul class="plan-features" v-if="p.features && p.features.length">
                <li v-for="f in p.features" :key="f">{{ f }}</li>
              </ul>

              <div class="plan-cta">
                <a class="btn-secondary" :href="`/purchase?plan=${p.id}`">Purchase</a>
              </div>
            </div>
          </div>
        </div>
      ...
      <!-- FAQ -->
      <section v-if="page.faqs && page.faqs.length" class="faq-section">
        <div class="container">
          <h2 class="section-title">FAQ</h2>
          <div class="faq-list">
            <div v-for="q in page.faqs" :key="q.id" class="faq-item">
