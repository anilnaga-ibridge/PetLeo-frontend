...
// Enable reactivity if initialData is updated later
watch(() => props.initialData, newVal => {
  if (newVal) {
    ...
    if (formDefinition.value?.fields) {
      formDefinition.value.fields.forEach(field => {
        if (data[field.field_key] === undefined) {
          if (field.field_type === 'BOOLEAN') data[field.field_key] = false
          else if (field.field_type === 'MULTISELECT') data[field.field_key] = []
          else data[field.field_key] = null
        }
      })
    }
    formData.value = data
