<script setup>
import { ref, computed, onMounted, onUnmounted, watch } from 'vue'
import axios from 'axios'
import { useIdleTimer } from '@/composables/useIdleTimer'
import { useRouter } from 'vue-router'
import MultiBoxPinInput from '@/components/MultiBoxPinInput.vue'
import { usePermissionStore } from '@/stores/permissionStore'
import { storeToRefs } from 'pinia'
import { useAuthStore } from '@/stores/authStore'
import { useBiometric } from '@/composables/useBiometric'

const { isLocked } = useIdleTimer()
const router = useRouter()
const pin = ref('')
const loading = ref(false)
const error = ref('')

// Biometric authentication
const { biometricAvailable, biometricLoading, biometricError, checkAvailability, authenticate } = useBiometric()

const API_BASE = import.meta.env.VITE_API_BASE_URL || 'http://127.0.0.1:8000'

const permissionStore = usePermissionStore()
const authStore = useAuthStore()
const { userData: user } = storeToRefs(permissionStore)

// Dynamic PIN length based on user's stored pin_length
const pinLength = ref(4) // Default to 4

const fetchPinLength = async () => {
  // Check both storages, prioritize localStorage as it persists longer, but check session if local is empty
  let userData = JSON.parse(localStorage.getItem('userData') || 'null')
  if (!userData) {
    userData = JSON.parse(sessionStorage.getItem('userData') || '{}')
  }

  // Backup phone source if userData is incomplete
  // Priority: 1. Dedicated lock_screen_phone, 2. Storage userData, 3. Store user ref
  const phone = localStorage.getItem('lock_screen_phone') || sessionStorage.getItem('lock_screen_phone') || 
                userData.phone_number || userData.phoneNumber || 
                user.value?.phone_number || user.value?.phoneNumber || '' 
  
  // Try to use stored PIN length first
  if (userData.pin_length) {
    pinLength.value = Number(userData.pin_length)
  } else if (user.value?.pin_length) {
    pinLength.value = Number(user.value.pin_length)
  }
  
  // Always verify/update from backend if phone number exists
  if (phone) {
    try {
      const res = await axios.get(`${API_BASE}/auth/api/auth/check-pin-length/`, {
        params: { phone_number: phone },
      })
      
      if (res.data?.pin_length) {
        const length = Number(res.data.pin_length)
        if (!isNaN(length) && length > 0) {
          pinLength.value = length
          
          // Update BOTH storages to be safe
          if (localStorage.getItem('userData')) {
            const localData = JSON.parse(localStorage.getItem('userData') || '{}')

            localData.pin_length = length
            localStorage.setItem('userData', JSON.stringify(localData))
          }
          if (sessionStorage.getItem('userData')) {
            const sessionData = JSON.parse(sessionStorage.getItem('userData') || '{}')

            sessionData.pin_length = length
            sessionStorage.setItem('userData', JSON.stringify(sessionData))
          }
        }
      }
    } catch (e) {
      console.error('Failed to fetch PIN length for lock screen', e)
    }
  }
}

// Check PIN length when component mounts OR when screen locks

watch(pinLength, () => {
  pin.value = ''
  error.value = ''
})

// Check PIN length when component mounts OR when screen locks OR when user changes
watch([isLocked, user], async ([newLocked, newUser], [oldLocked, oldUser]) => {
  if (newLocked) {
    await fetchPinLength()
    checkAvailability()
  }
  
  // If user changed (login/logout/switch), refresh PIN length
  if (JSON.stringify(newUser) !== JSON.stringify(oldUser)) {
    if (!newUser) {
      pinLength.value = 4 // Reset to default on logout
    } else {
      await fetchPinLength() // Fetch for new user
    }
  }
})

onMounted(async () => {
  await fetchPinLength()
  await checkAvailability()
})

const unlock = async () => {
  if (pin.value.length < pinLength.value) return
  
  loading.value = true
  error.value = ''
  
  try {
    console.log("🔓 Unlocking with:", { pin: pin.value, action: 'reverify' })

    await axios.post(`${API_BASE}/auth/api/auth/login-with-pin/`, {
      pin: pin.value,
      action: 'reverify',
    }, {
      headers: { Authorization: `Bearer ${useCookie('accessToken').value}` },
    })
    
    // Unlock success with exit animation
    const card = document.querySelector('.glass-card')
    if (card) {
      card.style.transform = 'scale(1.1) translateY(-20px)'
      card.style.opacity = '0'
    }
    
    setTimeout(() => {
      isLocked.value = false
      pin.value = ''
    }, 300)
    
  } catch (err) {
    console.error("🔓 Unlock Error:", err)
    if (err.response) {
      error.value = err.response.data.detail || "Invalid PIN"
    } else {
      error.value = "Backend not responding"
    }
    pin.value = ''
  } finally {
    loading.value = false
  }
}

const unlockWithBiometric = async () => {
  error.value = ''
  
  const result = await authenticate()
  
  if (result.success) {
    // Biometric verified, unlock the screen
    const card = document.querySelector('.glass-card')
    if (card) {
      card.style.transform = 'scale(1.1) translateY(-20px)'
      card.style.opacity = '0'
    }
    
    setTimeout(() => {
      isLocked.value = false
    }, 300)
  } else {
    error.value = result.error || 'Biometric authentication failed. Please use PIN.'
  }
}


// Canvas Particle Logic
const canvasRef = ref(null)
let animationFrameId
let resize
let particles = []

onMounted(() => {
  const canvas = canvasRef.value
  if (!canvas) return
  
  const ctx = canvas.getContext('2d')
  
  resize = () => {
    if (canvas) {
      canvas.width = window.innerWidth
      canvas.height = window.innerHeight
    }
  }
  
  window.addEventListener('resize', resize)
  resize()
  
  class Particle {
    constructor() {
      this.x = Math.random() * canvas.width
      this.y = Math.random() * canvas.height
      this.vx = (Math.random() - 0.5) * 0.5 // Moderate speed
      this.vy = (Math.random() - 0.5) * 0.5
      this.size = Math.random() * 2 + 1
    }
    
    update() {
      this.x += this.vx
      this.y += this.vy
      
      // Wrap around screen
      if (this.x < 0) this.x = canvas.width
      if (this.x > canvas.width) this.x = 0
      if (this.y < 0) this.y = canvas.height
      if (this.y > canvas.height) this.y = 0
    }
    
    draw() {
      ctx.fillStyle = 'rgba(255, 255, 255, 0.3)'
      ctx.beginPath()
      ctx.arc(this.x, this.y, this.size, 0, Math.PI * 2)
      ctx.fill()
    }
  }
  
  // Create particles
  particles = []
  for (let i = 0; i < 50; i++) {
    particles.push(new Particle())
  }
  
  const animate = () => {
    if (!canvas) return
    ctx.clearRect(0, 0, canvas.width, canvas.height)
    
    particles.forEach((p, index) => {
      p.update()
      p.draw()
      
      // Connect particles
      for (let j = index + 1; j < particles.length; j++) {
        const p2 = particles[j]
        const dx = p.x - p2.x
        const dy = p.y - p2.y
        const dist = Math.sqrt(dx * dx + dy * dy)
        
        if (dist < 150) {
          ctx.strokeStyle = `rgba(255, 255, 255, ${0.1 - dist/1500})`
          ctx.lineWidth = 1
          ctx.beginPath()
          ctx.moveTo(p.x, p.y)
          ctx.lineTo(p2.x, p2.y)
          ctx.stroke()
        }
      }
    })
    
    animationFrameId = requestAnimationFrame(animate)
  }
  
  animate()
})

onUnmounted(() => {
  cancelAnimationFrame(animationFrameId)
  if (resize) {
    window.removeEventListener('resize', resize)
  }
})

const logout = async () => {
  console.log("🔒 Logout clicked - Clearing session...")
  loading.value = true 

  try {
    // Clear lock state first
    isLocked.value = false
    localStorage.removeItem('isAppLocked')

    // Use centralized logout which handles tokens, ability, and redirect
    await authStore.logout()
  } catch (err) {
    console.error("Logout failed:", err)

    // Fallback if store fails
    window.location.href = '/login'
  } finally {
    loading.value = false
  }
}
</script>

<template>
  <div
    v-if="isLocked"
    class="lock-overlay"
  >
    <!-- Animated Particle Background -->
    <canvas
      ref="canvasRef"
      class="particle-canvas"
    />

    <div class="lock-content">
      <div class="text-center mb-8">
        <VBadge
          location="bottom right"
          offset-x="10"
          offset-y="10"
          color="error"
          icon="tabler-lock"
          class="mb-6 avatar-pulse"
        >
          <VAvatar
            color="primary"
            size="96"
            class="elevation-10 ring-4 ring-white ring-opacity-20"
          >
            <VImg
              v-if="user && user.avatar"
              :src="user.avatar"
              alt="User Avatar"
            />
            <span
              v-else-if="user && user.fullName"
              class="text-h2 font-weight-bold text-white"
            >
              {{ user.fullName.charAt(0).toUpperCase() }}
            </span>
            <VIcon
              v-else
              icon="tabler-user"
              size="48"
              color="white"
            />
          </VAvatar>
        </VBadge>
        <h2 class="text-h3 font-weight-bold text-white mb-2 tracking-tight typing-effect">
          Welcome Back
        </h2>
        <p class="text-white text-opacity-90 text-subtitle-1 font-weight-medium fade-in-up">
          {{ user && user.fullName || 'User' }}
        </p>
      </div>

      <VCard
        width="460"
        class="pa-8 pb-10 rounded-xl elevation-24 glass-card"
      >
        <p class="text-center text-body-1 mb-6 text-medium-emphasis font-weight-medium">
          Enter your PIN to unlock screen
        </p>

        <VAlert
          v-if="error"
          type="error"
          variant="tonal"
          density="comfortable"
          class="mb-6"
          icon="tabler-alert-circle"
        >
          {{ error }}
        </VAlert>

        <div class="mb-8 d-flex justify-center">
          <MultiBoxPinInput
            v-model="pin"
            :length="pinLength"
            :error="!!error"
            @complete="unlock"
          />
        </div>

        <VBtn
          v-if="biometricAvailable"
          block
          variant="tonal"
          color="secondary"
          size="x-large"
          rounded="lg"
          prepend-icon="tabler-fingerprint"
          :loading="biometricLoading"
          class="mb-4 font-weight-bold"
          @click="unlockWithBiometric"
        >
          Unlock with Fingerprint
        </VBtn>

        <VBtn
          block
          color="primary"
          size="x-large"
          rounded="lg"
          prepend-icon="tabler-key"
          :loading="loading"
          class="mb-6 font-weight-bold text-button btn-gradient"
          elevation="4"
          @click="unlock"
        >
          Unlock with PIN
        </VBtn>

        <div class="d-flex justify-center gap-4">
          <VBtn
            variant="text"
            color="primary"
            size="small"
            class="text-caption font-weight-bold"
            @click="router.push('/login')"
          >
            Forgot PIN?
          </VBtn>
          <span class="text-disabled">|</span>
          <VBtn
            variant="text"
            color="error"
            size="small"
            class="text-caption font-weight-bold"
            @click="logout"
          >
            Logout
          </VBtn>
        </div>
      </VCard>
    </div>
  </div>
</template>

<style scoped>
.lock-overlay {
  position: fixed;
  top: 0;
  left: 0;
  width: 100vw;
  height: 100vh;
  background: linear-gradient(135deg, #0f172a 0%, #1e293b 100%);
  backdrop-filter: blur(15px);
  z-index: 2147483647 !important;
  display: flex;
  align-items: center;
  justify-content: center;
  overflow: hidden;
}

/* Particle Canvas */
.particle-canvas {
  position: absolute;
  top: 0;
  left: 0;
  width: 100%;
  height: 100%;
  pointer-events: none; /* Let clicks pass through */
  z-index: 1;
}

.lock-content {
  position: relative;
  z-index: 10;
  display: flex;
  flex-direction: column;
  align-items: center;
}

/* Pulse Animation for Avatar */
.avatar-pulse {
  animation: pulse-glow 3s infinite;
}

@keyframes pulse-glow {
  0% { box-shadow: 0 0 0 0 rgba(255, 255, 255, 0.4); }
  70% { box-shadow: 0 0 0 15px rgba(255, 255, 255, 0); }
  100% { box-shadow: 0 0 0 0 rgba(255, 255, 255, 0); }
}

/* Premium Glass Card */
.glass-card {
  background: rgba(255, 255, 255, 0.4) !important;
  backdrop-filter: blur(20px) saturate(180%);
  border: 1px solid rgba(255, 255, 255, 0.5);
  box-shadow: 0 40px 100px -20px rgba(0, 0, 0, 0.3), inset 0 0 20px rgba(255, 255, 255, 0.2);
  transform-style: preserve-3d;
  perspective: 1000px;
}

.v-theme--dark .glass-card {
  background: rgba(15, 23, 42, 0.6) !important;
  border: 1px solid rgba(255, 255, 255, 0.08);
  box-shadow: 0 40px 100px -20px rgba(0, 0, 0, 0.5), inset 0 0 40px rgba(255, 255, 255, 0.02);
}

/* Gradient Button */
.btn-gradient {
  background: linear-gradient(135deg, #4f46e5 0%, #7c3aed 100%) !important;
  color: white !important;
  box-shadow: 0 10px 15px -3px rgba(79, 70, 229, 0.4);
  transition: all 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275);
}

.btn-gradient:hover {
  transform: translateY(-3px) scale(1.02);
  box-shadow: 0 15px 25px -5px rgba(79, 70, 229, 0.5);
}

@keyframes slideUpFade {
  from {
    opacity: 0;
    transform: translateY(40px) scale(0.95);
  }
  to {
    opacity: 1;
    transform: translateY(0) scale(1);
  }
}

/* Typing Effect */
.typing-effect {
  overflow: hidden;
  white-space: nowrap;
  border-right: 3px solid rgba(255, 255, 255, 0.75);
  display: inline-block;
  margin: 0 auto;
  animation: 
    typing 2s steps(20, end),
    blink-caret .75s step-end infinite;
}

@keyframes typing {
  from { width: 0 }
  to { width: 100% }
}

@keyframes blink-caret {
  from, to { border-color: transparent }
  50% { border-color: rgba(255, 255, 255, 0.75); }
}

/* Fade In Up */
.fade-in-up {
  animation: fadeInUp 1s ease-out forwards;
  opacity: 0;
  text-shadow: 0 2px 4px rgba(0,0,0,0.3);
  animation-delay: 0.5s;
}

@keyframes fadeInUp {
  from {
    opacity: 0;
    transform: translateY(20px);
  }
  to {
    opacity: 1;
    transform: translateY(0);
  }
}

:deep(input::-webkit-outer-spin-button),
:deep(input::-webkit-inner-spin-button) {
  -webkit-appearance: none;
  margin: 0;
}
</style>
