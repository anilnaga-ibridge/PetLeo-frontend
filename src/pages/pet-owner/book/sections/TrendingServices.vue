      ...
      <section class="trending-section py-20 bg-slate-50">
        <VContainer>
          <div class="section-header mb-12 px-4 px-md-0 d-flex align-end justify-space-between">
            <div>
              <h2 class="text-h3 font-weight-black text-slate-900 mb-2 tracking-tight">Trending Services</h2>
              <p class="text-body-1 text-slate-500 font-medium max-w-md">The most sought-after pet care services in your community this month.</p>
            </div>
            <VBtn variant="text" color="primary" class="font-weight-bold d-none d-sm-flex">
              Explore All <VIcon icon="tabler-arrow-right" size="18" class="ml-2" />
            </VBtn>
          </div>

          <VRow v-if="loading" class="g-6">
            <VCol v-for="i in 5" :key="i" cols="12" sm="6" md="4" lg="2.4">
              <VSkeletonLoader type="image, heading" class="rounded-xl" />
            </VCol>
          </VRow>
          ...
          <VRow v-else class="g-6">
            ...
            >
              <div class="trending-card-premium pa-8 bg-white border rounded-3xl d-flex flex-column align-center text-center cursor-pointer h-100 position-relative overflow-hidden">
                <div class="trending-badge-floating" :class="item.color">{{ item.badge }}</div>
                
                <div class="icon-wrapper mb-6" :class="item.color">
                   <div class="icon-blob"></div>
                   <VIcon :icon="item.icon" size="36" class="position-relative z-1" />
                </div>
                
                <h3 class="text-h5 font-weight-black text-slate-900 mb-3 leading-tight">{{ item.title }}</h3>
