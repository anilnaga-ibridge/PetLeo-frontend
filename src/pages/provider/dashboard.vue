...
<template>
  <ProviderLayout>
    <div class="pa-6">
      ...
      <VRow v-if="isEmployee" class="mb-6">
        <VCol cols="12">
          <VCard color="primary" class="welcome-banner">
            <VRow no-gutters>
              <VCol cols="12" md="8" class="pa-6">
                <h2 class="text-h3 font-weight-bold text-white mb-2">Ready for your shift?</h2>
                ...
                <VBtn color="white" class="text-primary font-weight-bold" :to="{ name: 'provider-customer-bookings' }">
                  View Bookings
                </VBtn>
              </VCol>
              <VCol cols="12" md="4" class="d-none d-md-flex align-center justify-center">
                 <VIcon icon="tabler-calendar-user" size="120" color="white" class="opacity-20" />
              </VCol>
            </VRow>
          </VCard>
        </VCol>
      ...
      <VRow v-if="!isEmployee">
        ...
        <VCol cols="12" md="4">
          <VCard class="analytic-card overflow-hidden">
            <div class="pa-5 d-flex align-center gap-4">
              <VAvatar color="primary" variant="tonal" size="58" rounded="lg">
                <VIcon icon="tabler-building-hospital" size="28" />
              </VAvatar>
              <div>
                <div class="text-h3 font-weight-bold">{{ summary.total_clinics || 0 }}</div>
                <div class="text-body-1 text-medium-emphasis">Active Clinics</div>
              </div>
            </div>
            <VDivider />
            <div class="pa-3 bg-light-primary text-center cursor-pointer" @click="$router.push({ name: 'provider-settings-tab', params: { tab: 'clinics' } })">
        ...
        <VCol cols="12" md="4">
          <VCard class="analytic-card overflow-hidden">
            <div class="pa-5 d-flex align-center gap-4">
              <VAvatar color="info" variant="tonal" size="58" rounded="lg">
                <VIcon icon="tabler-users" size="28" />
              </VAvatar>
              <div>
                <div class="text-h3 font-weight-bold">{{ summary.total_employees || 0 }}</div>
                <div class="text-body-1 text-medium-emphasis">Total Team Members</div>
              </div>
            </div>
            <VDivider />
        ...
        <VCol cols="12" md="4">
          <VCard class="analytic-card overflow-hidden">
            <div class="pa-5 d-flex align-center gap-4">
              <VAvatar color="success" variant="tonal" size="58" rounded="lg">
                <VIcon icon="tabler-shield-check" size="28" />
              </VAvatar>
              <div>
                <div class="text-h3 font-weight-bold">{{ summary.total_roles || 0 }}</div>
                <div class="text-body-1 text-medium-emphasis">Configured Roles</div>
              </div>
            </div>
            <VDivider />
      ...
      <VRow v-if="!isEmployee">
        ...
        <VCol cols="12" md="8">
          <VCard class="analytic-card h-100" elevation="2">
            <VCardItem>
              <template #append>
                <div class="d-flex align-center gap-2">
                   <VChip color="success" size="small" variant="tonal" class="font-weight-bold">
                      <VIcon start icon="tabler-trending-up" />
                      +12.5%
                   </VChip>
                   <VBtn icon="tabler-dots-vertical" variant="text" size="small" color="medium-emphasis" />
                </div>
              </template>
              <VCardTitle class="text-h5 font-weight-bold">Booking Trends</VCardTitle>
              <VCardSubtitle>Last 7 Days Performance</VCardSubtitle>
            </VCardItem>
            
            <VCardText class="pb-0" style="min-height: 250px;">
               <VueApexCharts
                  type="area"
                  height="250"
            ...
            <div class="d-flex justify-space-between pa-4">
               <div class="text-center">
                  <div class="text-h4 font-weight-black">{{ summary.bookings?.total_bookings || 0 }}</div>
                  <div class="text-caption text-medium-emphasis font-weight-bold uppercase">Total</div>
               </div>
               <div class="text-center">
                  <div class="text-h4 font-weight-black text-warning">{{ summary.bookings?.pending_bookings || 0 }}</div>
                  <div class="text-caption text-medium-emphasis font-weight-bold uppercase">Pending</div>
               </div>
               <div class="text-center">
                  <div class="text-h4 font-weight-black text-success">{{ summary.bookings?.completed_bookings || 0 }}</div>
                  <div class="text-caption text-medium-emphasis font-weight-bold uppercase">Completed</div>
               </div>
            </div>
          </VCard>
        </VCol>
        ...
        <VCol cols="12" md="4">
          <VCard class="analytic-card h-100" elevation="2">
            <VCardItem>
               <VCardTitle class="text-h5 font-weight-bold">Client Satisfaction</VCardTitle>
            ...
            <VCardText class="d-flex flex-column align-center justify-center py-6">
                ...
                <div class="position-relative d-flex align-center justify-center mb-6" style="height: 150px; width: 150px;">
                    <VProgressCircular
                    ...
                    >
                      <div class="text-center">
                         <div class="text-h3 font-weight-black text-slate-800">{{ summary.average_rating || 0 }}</div>
                         <div class="d-flex justify-center">
                           <VIcon icon="tabler-star-filled" color="warning" size="20" />
                         </div>
                      </div>
                    </VProgressCircular>
                </div>
                ...
                <div class="w-100 px-2">
                   <div v-for="star in [5, 4, 3, 2, 1]" :key="star" class="d-flex align-center gap-2 mb-2">
                      <div class="text-caption font-weight-bold text-slate-500 min-w-30">{{ star }} Star</div>
                      <VProgressLinear
                ...
                <div class="mt-6 w-100">
                  <VBtn block variant="outlined" :to="{ name: 'provider-reviews' }">
                    View All Reviews
                  </VBtn>
                </div>
            </VCardText>
      ...
      <VRow v-if="canViewBilling">
        <VCol cols="12">
          <VCard class="plan-card" elevation="2">
            <VRow no-gutters>
              <VCol cols="12" md="8" class="pa-6 border-e">
                <div class="d-flex align-center gap-3 mb-4">
                  <VAvatar color="primary" size="40">
                    <VIcon icon="tabler-crown" color="white" />
                  </VAvatar>
                  <h2 class="text-h4 font-weight-bold">Current Plan: {{ permissionStore.planDetails?.title || 'Basic' }}</h2>
                </div>
              ...
              <VCol cols="12" md="4" class="pa-6 bg-var-theme-background d-flex flex-column justify-center align-center text-center">
                <div class="text-caption mb-1">Plan Expires On</div>
      ...
      <VRow>
        ...
        <VCol cols="12">
          <div class="d-flex align-center justify-space-between mb-6">
            <h3 class="text-h4 font-weight-bold">{{ isEmployee ? 'My Assigned Services' : 'Active Modules' }}</h3>
          ...
          <VRow>
                ...
                <VCardItem class="pb-2">
                  <template #prepend>
                    <VAvatar
                      ...
                      rounded="lg"
                    >
                      <VIcon :icon="service.icon || 'tabler-app-window'" size="24" />
                    </VAvatar>
                  </template>
                  <VCardTitle class="text-h5 font-weight-bold">{{ service.service_name }}</VCardTitle>
                  <VCardSubtitle>{{ service.service_key }}</VCardSubtitle>
                </VCardItem>
                
                <VCardText>
                  <div class="mb-4">
                    <div class="text-overline mb-2">Capabilities</div>
            ...
            <VCol v-if="!hasServices" cols="12">
              <VCard variant="tonal" color="info" class="pa-10 text-center">
                <VIcon icon="tabler-info-circle" size="48" class="mb-4" />
                <h3 class="text-h4 mb-2">Configure Your Modules</h3>
                <p class="text-body-1 mx-auto" style="max-width: 500px;">
                ...
                <VBtn color="primary" class="mt-4" :to="{ name: 'provider-home' }">
                  Start Building Plan
                </VBtn>
              </VCard>
            </VCol>
