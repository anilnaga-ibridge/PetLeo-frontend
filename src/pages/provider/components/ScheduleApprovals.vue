...
<template>
  <div>
    <!-- Empty State -->
    <div v-if="!loading && groupedByEmployee.length === 0" class="text-center py-12">
      <VIcon icon="tabler-circle-check" size="56" color="success" class="mb-4 opacity-40" />
      <h3 class="text-h5 font-weight-bold text-slate-700">All caught up!</h3>
      <p class="text-body-2 text-slate-400 mt-2">No pending schedule approvals from your team.</p>
    </div>

    <VProgressLinear v-if="loading" indeterminate color="primary" class="mb-6" />

    <!-- Employee Groups -->
    <div v-for="group in groupedByEmployee" :key="group.empId" class="mb-6 approval-card">
      <VCard flat border class="rounded-2xl overflow-hidden">
        <!-- Employee Header -->
        <div class="d-flex align-center justify-space-between pa-6 border-b bg-slate-50">
          <div class="d-flex align-center gap-4">
            ...
            <div>
              <div class="text-subtitle-1 font-weight-black text-slate-800">{{ group.empName }}</div>
              <div class="text-caption text-slate-400">{{ group.schedules.length }} days submitted</div>
            </div>
          </div>
        ...
        <VTable density="compact">
          ...
          <tbody>
            <tr v-for="sched in group.schedules.sort((a,b) => new Date(a.date) - new Date(b.date))" :key="sched.id">
              <td class="font-weight-bold">{{ sched.date }}</td>
              <td>
                <VChip v-if="sched.start_time === sched.end_time" size="x-small" color="secondary" variant="tonal">Off</VChip>
                <VChip v-else size="x-small" color="success" variant="tonal">Working</VChip>
              </td>
              <td>
                <span v-if="sched.start_time !== sched.end_time">
                  {{ formatTime(sched.start_time) }} - {{ formatTime(sched.end_time) }}
                  <VChip size="x-small" color="primary" variant="text" class="ml-1">
                    ({{
                      (() => {
              ...
              <td>
                <div class="text-caption text-truncate" style="max-width: 250px" :title="sched.reason">
                  {{ sched.reason || '—' }}
                </div>
              </td>
              <td>
                ...
                >
                  <VIcon icon="tabler-eye" size="14" class="mr-1" />
                  Slots
                </VBtn>
                <div v-else class="text-caption text-slate-300">N/A</div>
              </td>
            </tr>
          </tbody>
        </VTable>
    ...
    <VDialog v-model="rejectDialog" max-width="480">
      <VCard title="Reject Schedule">
        ...
        <VCardActions class="pa-4">
          <VSpacer />
          <VBtn variant="tonal" @click="rejectDialog = false">Cancel</VBtn>
          <VBtn color="error" @click="rejectAll(targetGroup)">Confirm Reject</VBtn>
        </VCardActions>
      </VCard>
    </VDialog>
