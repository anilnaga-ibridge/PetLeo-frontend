...
<template>
  <VCard title="My Services" subtitle="Configure which services appear on your public profile and customize their details.">
    <VCardText v-if="loading" class="text-center pa-12">
      <VProgressCircular indeterminate color="primary" />
    </VCardText>
    
    <VCardText v-else>
      <VList lines="two" class="service-list">
        <template v-for="service in allowedServices" :key="service.service_id">
          <VListItem>
            <template #prepend>
            ...
            <VExpandTransition>
              <div v-if="getProviderService(service.service_id).is_active" class="mt-4 border rounded pa-4 bg-slate-50">
                <VRow>
                  <VCol cols="12" md="8">
                    <AppTextarea 
