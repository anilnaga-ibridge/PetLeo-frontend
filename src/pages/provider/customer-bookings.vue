...
const activeTab = ref(route.query.status?.toUpperCase() || 'ALL')
const searchQuery = ref(route.query.search || '')
const serviceIdFilter = computed(() => route.query.service_id)
...
const filteredBookings = computed(() => {
  ...
  if (serviceIdFilter.value) {
    result = result.filter(b => b.service_id === serviceIdFilter.value)
  }
...
const fetchBookings = async () => {
  ...
  try {
    ...
    if (serviceIdFilter.value) {
      url += (url.includes('?') ? '&' : '?') + `service_id=${serviceIdFilter.value}`
    }
    ...
    <div class="customer-bookings-page pa-6 pb-16">
      ...
      <VRow class="mb-8">
        ...
        >
          <VCard
            flat
            class="glass-card pa-6 d-flex align-center justify-space-between"
          >
            <div>
                  ...
                  <div class="d-flex align-center gap-4 flex-grow-1 border-s-md pl-md-6 border-slate-100">
                    ...
                    >
                      <div class="text-center">
                        <div class="text-caption font-weight-black text-uppercase lh-1">
                          {{ new Date(booking.selected_time).toLocaleDateString('en-GB', { month: 'short' }) }}
                        </div>
                        <div class="text-h6 font-weight-black lh-1">
                          {{ new Date(booking.selected_time).getDate() }}
                        </div>
                      </div>
                    </VAvatar>
                    <div>
