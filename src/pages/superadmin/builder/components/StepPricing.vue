...
<template>
  <div>
            ...
            <VCardText class="pa-5 d-flex flex-column h-100">
              ...
              <div class="d-flex flex-column gap-2 mb-3 flex-grow-1">
                <div class="d-flex align-center gap-3">
                  <VAvatar size="28" color="primary" variant="tonal" class="rounded">
                    <VIcon icon="tabler-hierarchy-2" size="16" />
                  </VAvatar>
                  <div>
                    <div class="text-caption text-medium-emphasis">Service Type</div>
                    <div class="text-body-2 font-weight-medium">{{ pricing.category_display || 'All Types' }}</div>
                  </div>
                </div>

                <div class="d-flex align-center gap-3">
                  <VAvatar size="28" color="info" variant="tonal" class="rounded">
                    <VIcon icon="tabler-building-hospital" size="16" />
                  </VAvatar>
                  <div>
                    <div class="text-caption text-medium-emphasis">Facility</div>
                    <div class="text-body-2 font-weight-medium">{{ pricing.facility_display || 'All Facilities' }}</div>
                  </div>
                </div>

                <div v-if="pricing.duration_minutes" class="d-flex align-center gap-3">
                  <VAvatar size="28" color="warning" variant="tonal" class="rounded">
                    <VIcon icon="tabler-clock" size="16" />
                  </VAvatar>
                  <div>
                    <div class="text-caption text-medium-emphasis">Duration</div>
                    <div class="text-body-2 font-weight-medium">{{ pricing.duration_minutes }} Minutes</div>
                  </div>
                </div>
              </div>
              ...
              <div class="d-flex align-center justify-space-between pt-3 border-t mt-auto" @click.stop>
                <div class="d-flex align-center gap-2">
                  <VBadge
                    dot
                    :color="pricing.is_active ? 'success' : 'error'"
                    inline
                  />
      ...
      <VCard v-else class="border">
        ...
        >
          ...
          <template #item.billing_unit="{ item }">
            <VChip size="small" label>{{ getBillingLabel(item.billing_unit) }}</VChip>
          </template>
    ...
    <div v-else class="text-center py-12 bg-light rounded-lg border-dashed">
      <VIcon icon="tabler-currency-dollar" size="48" color="medium-emphasis" class="mb-4" />
      <h3 class="text-h6 font-weight-bold mb-2">No Pricing Rules</h3>
      <p class="text-body-2 text-medium-emphasis mb-6">Define how much providers should be charged for this service.</p>
      <VBtn variant="tonal" color="primary" @click="openAddDrawer">Add First Rule</VBtn>
    </div>
    ...
    <VDialog v-model="deleteDialog" width="420" transition="dialog-bottom-transition" persistent>
      <VCard class="pa-4 rounded-xl" elevation="12">
        ...
        <div class="d-flex justify-end gap-2">
          ...
          <VBtn color="red" prepend-icon="tabler-trash" @click="deletePricing">
            Delete
          </VBtn>
        </div>
      </VCard>
      ...
      >
        <div class="d-flex flex-column h-100">
          <div class="pa-6 border-b d-flex justify-space-between align-center bg-surface sticky-header">
            <h3 class="text-h6 font-weight-bold">{{ isEdit ? 'Edit Pricing' : 'Add Pricing' }}</h3>
            <VBtn icon="tabler-x" variant="text" @click="drawerOpen = false" />
          </div>

          <div class="flex-grow-1 overflow-y-auto pa-6">
            <VForm id="pricingForm" @submit.prevent="submit">
              <VCard variant="tonal" color="primary" class="pa-4 mb-6 border-0">
                <h4 class="text-subtitle-1 font-weight-bold mb-4">Pricing Context</h4>
              ...
              <VCard variant="tonal" color="secondary" class="pa-4 mb-6 border-0">
                <h4 class="text-subtitle-1 font-weight-bold mb-4">Pricing Details</h4>
          ...
          <div class="pa-6 border-t bg-surface">
            <div class="d-flex justify-end gap-4">
              ...
              <VBtn color="primary" type="submit" form="pricingForm">
