...
const onDrop = (e, idx) => {
  ...
  const to = idx;
  if (from === to) { cleanupDragClasses(); return; }
      ...
      <VCardText>
        <VRow class="align-end" no-gutters>
          ...
          <VCol cols="12" sm="4" class="d-flex justify-sm-end mt-4 mt-sm-0 gap-3">
            ...
            <VBtn size="small" :style="{ background:'#6C27FF' }" @click="openCreateModal">+ Create</VBtn>
              ...
              >
                <div>
                  <div class="d-flex align-center flex-wrap gap-2">
                    ...
                    <VChip size="small" label :style="{ background:'#EFE4FF', color:'#6C27FF' }">{{ d.field_type }}</VChip>
                  </div>
                ...
                <div class="d-flex flex-column gap-2">
                  <VBtn size="x-small" :style="{ background:'#6C27FF' }" @click="openEditModal(d)">Edit</VBtn>
    ...
    <VDialog v-model="modalOpen" max-width="650">
      <VCard elevation="8" class="rounded-lg">
        <VCardTitle class="d-flex justify-space-between align-center px-5 py-4" style="background:#F5EDFF; border-bottom:1px solid #E8D8FF;">
          <span class="text-h6 font-weight-bold" style="color:#6C27FF">{{ isEdit ? "Edit Field" : "Create Field" }}</span>
          <VBtn icon variant="text" @click="closeModal"><VIcon icon="mdi-close" size="26" color="purple-darken-2" /></VBtn>
        </VCardTitle>

        <VCardText class="px-5 py-4">
          <VRow>
            <VCol cols="12" sm="6">
              <VTextField v-model="form.name" label="Machine Name *" placeholder="business_license_no" variant="outlined" density="comfortable" />
            </VCol>

            <VCol cols="12" sm="6">
              <VTextField v-model="form.label" label="Label *" placeholder="UI label" variant="outlined" density="comfortable" />
            </VCol>

            <VCol cols="12" sm="6">
              <VSelect v-model="form.field_type" :items="fieldTypes" item-title="label" item-value="value" label="Field Type *" variant="outlined" density="comfortable" />
            </VCol>

            <VCol cols="12" sm="6">
              <VTextField v-model.number="form.order" label="Order" placeholder="0" type="number" variant="outlined" density="comfortable" />
            </VCol>
          </VRow>
          ...
          <div v-if="['dropdown','multiselect'].includes(form.field_type)" class="mt-4">
            ...
            <div class="d-flex gap-3">
              <VTextField v-model="optionInput" label="Add Option" placeholder="Type option and Enter" variant="outlined" density="comfortable" class="flex-grow-1" @keyup.enter="addOptionToForm" />
              <VBtn size="small" style="background:#6C27FF" @click="addOptionToForm">Add</VBtn>
            </div>

            <div class="d-flex flex-wrap gap-2 mt-3">
              <VChip v-for="o in form.options" :key="o" closable class="bg-purple-lighten-5 text-purple-darken-3" @click:close="removeOptionFromForm(o)" size="small">{{ o }}</VChip>
            </div>
          </div>

          <VTextField v-model="form.help_text" label="Help Text" placeholder="Optional guidance" variant="outlined" density="comfortable" class="mt-4" />
          <VCheckbox v-model="form.is_required" label="Required Field" color="purple-darken-2" class="mt-1" />
        </VCardText>

        <VCardActions class="px-5 py-4 justify-end" style="border-top:1px solid #EFE8FF">
          <VBtn variant="text" size="small" @click="closeModal">Cancel</VBtn>
          <VBtn size="small" class="purple-btn" @click="saveField">{{ isEdit ? "Update Field" : "Create Field" }}</VBtn>
        </VCardActions>
      </VCard>
    </VDialog>
