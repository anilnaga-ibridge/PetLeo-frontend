...
const onDocDrop = (e, idx) => {
  ...
  const to = idx;
  if (from === to) { cleanupDocDragClasses(); return; }
...
<template>
  ...
  <div class="d-flex justify-space-between align-center">
    ...
    <VCard elevation="2" class="mb-6">
      <VCardText>
        <VRow class="align-end" no-gutters>
          ...
          <!-- SEARCH -->
          <VCol cols="12" sm="5" class="pe-sm-3">
          ...
          <!-- ACTIONS -->
          <VCol cols="12" sm="4" class="d-flex justify-sm-end mt-4 mt-sm-0 gap-3">
            


            <VBtn size="small" :style="{ background:'#6C27FF' }" @click="openDocCreateModal">+ Create</VBtn>
    ...
    <VRow>
      ...
      <VCol cols="12" md="8">
        <VCard elevation="2">
          <VCardText>
            <!-- Loading -->
            <div v-if="docLoading" class="text-center py-10">
              <VProgressCircular indeterminate :color="'#6C27FF'" />
              <div class="mt-3">Loading documents…</div>
            </div>

            <!-- Empty -->
            <div v-else-if="docFiltered.length === 0" class="text-center py-10 text-grey">
              No documents found.
            </div>
              ...
              >
                <div>
                  <div class="d-flex align-center flex-wrap gap-2">
                    ...
                    <VChip size="small" label :style="{ background:'#EFE4FF', color:'#6C27FF' }">{{ d.allow_multiple ? 'multiple' : 'single' }}</VChip>
                  </div>
                  ...
                  <div class="mt-2 text-caption text-grey-darken-1 d-flex flex-wrap gap-4">
                    ...
                    <div v-if="d.allowed_types?.length">Types: {{ d.allowed_types.join(", ") }}</div>
                  </div>
                </div>

                <div class="d-flex flex-column gap-2">
                  <VBtn size="x-small" :style="{ background:'#6C27FF' }" @click="openDocEditModal(d)">Edit</VBtn>
                  <VBtn size="x-small" color="red-darken-2" @click="askDelete(d.id)">Delete</VBtn>
                </div>
              </div>

              <!-- pagination -->
              <div class="d-flex justify-center mt-6" v-if="docTotalPages > 1">
                <VBtn size="small" :style="{ background:'#6C27FF' }" @click="docGoFirst" :disabled="docPage === 1">«</VBtn>
                <VBtn size="small" :style="{ background:'#6C27FF' }" @click="docGoPrev" :disabled="docPage === 1">‹</VBtn>

                <div class="px-4 py-1 text-body-2">
                  Page {{ docPage }} / {{ docTotalPages }}
                </div>

                <VBtn size="small" :style="{ background:'#6C27FF' }" @click="docGoNext" :disabled="docPage === docTotalPages">›</VBtn>
                <VBtn size="small" :style="{ background:'#6C27FF' }" @click="docGoLast" :disabled="docPage === docTotalPages">»</VBtn>
              </div>
            </div>
          </VCardText>
        </VCard>
        ...
        <VCard elevation="3" class="pa-5 preview-card">
          <div class="d-flex align-center mb-4">
            <VAvatar size="36" color="purple-lighten-4">
              <VIcon icon="mdi-file-document-outline" color="purple-darken-2" />
            </VAvatar>
            <h3 class="text-h6 font-weight-bold ms-3 text-purple-darken-3">Preview & Tips</h3>
          </div>

          <div class="tip-box mb-4">
            <VIcon icon="mdi-drag" size="20" color="purple-darken-2" class="me-2" />
            <span>Drag documents to reorder. Save order after reordering.</span>
          </div>

          <div class="tip-box mb-4">
            <VIcon icon="mdi-file-check-outline" size="20" color="purple-darken-2" class="me-2" />
            <span>Pick common MIME types or add custom ones (e.g. application/vnd.geo+json).</span>
          </div>

          <VDivider class="my-4" />

          <h4 class="text-subtitle-1 font-weight-bold mb-3 text-purple-darken-3">Quick Validation</h4>

          <VList density="comfortable" nav class="rounded-lg">
            <VListItem>
              <template #prepend><VIcon icon="mdi-check-circle-outline" color="purple-darken-2" /></template>
    ...
    <!-- CREATE / EDIT MODAL -->
    <VDialog v-model="docModalOpen" max-width="720">
      <VCard elevation="8" class="rounded-lg">
        <VCardTitle class="d-flex justify-space-between align-center px-5 py-4" style="background:#F5EDFF; border-bottom:1px solid #E8D8FF;">
          <span class="text-h6 font-weight-bold" style="color:#6C27FF">{{ docIsEdit ? "Edit Document Definition" : "Create Document Definition" }}</span>
          <VBtn icon variant="text" @click="closeDocModal"><VIcon icon="mdi-close" size="26" color="purple-darken-2" /></VBtn>
        </VCardTitle>

        <VCardText class="px-5 py-4">
          <VRow>
            <VCol cols="12" sm="6">
              <VTextField v-model="docForm.key" label="Machine Key *" placeholder="id_proof" variant="outlined" density="comfortable" />
            </VCol>

            <VCol cols="12" sm="6">
              <VTextField v-model="docForm.label" label="Label *" placeholder="ID Proof" variant="outlined" density="comfortable" />
            </VCol>

            <VCol cols="12" sm="6">
              <VTextField v-model.number="docForm.order" label="Order" placeholder="0" type="number" variant="outlined" density="comfortable" />
            </VCol>

            <VCol cols="12" sm="6" class="d-flex align-center">
              <VCheckbox v-model="docForm.is_required" label="Required" color="purple-darken-2" />
              <VCheckbox v-model="docForm.allow_multiple" label="Allow multiple uploads" color="purple-darken-2" class="ms-4" />
            </VCol>
          </VRow>

          <!-- PREDEFINED MIME CHECKS -->
          <div class="mt-4">
            <h4 class="text-subtitle-2 font-weight-bold mb-2">Allowed File Types (quick select)</h4>
            <div class="d-flex flex-wrap gap-2">
              <VBtn v-for="m in PREDEFINED_MIMES" :key="m.value"
          ...
          <!-- MANUAL MIME ADD -->
          <div class="mt-4">
            <h4 class="text-subtitle-2 font-weight-bold mb-2">Add custom MIME type</h4>
            <div class="d-flex gap-3">
              <VTextField v-model="docTypeInput" label="e.g. application/vnd.geo+json" placeholder="Type MIME and press Add" variant="outlined" density="comfortable" class="flex-grow-1" @keyup.enter="addCustomMime" />
              <VBtn size="small" style="background:#6C27FF" @click="addCustomMime">Add</VBtn>
            </div>

            <div class="d-flex flex-wrap gap-2 mt-3">
              <VChip v-for="t in docForm.allowed_types" :key="t" closable class="bg-purple-lighten-5 text-purple-darken-3" @click:close="removeAllowedType(t)" size="small">{{ t }}</VChip>
            </div>
          </div>

          <VTextField v-model="docForm.help_text" label="Help Text" placeholder="Optional guidance" variant="outlined" density="comfortable" class="mt-4" />
        </VCardText>

        <VCardActions class="px-5 py-4 justify-end" style="border-top:1px solid #EFE8FF">
          <VBtn variant="text" size="small" @click="closeDocModal">Cancel</VBtn>
          <VBtn size="small" class="purple-btn" @click="saveDoc">{{ docIsEdit ? "Update" : "Create" }}</VBtn>
        </VCardActions>
      </VCard>
    </VDialog>

    <!-- Confirm delete dialog -->
    <VDialog v-model="confirmDialog.open" max-width="480">
      <VCard>
        <VCardTitle class="text-h6">Confirm Delete</VCardTitle>
        <VCardText>{{ confirmDialog.message }}</VCardText>
        <VCardActions class="justify-end">
          <VBtn variant="text" size="small" @click="confirmDialog.open = false">Cancel</VBtn>
          <VBtn color="red" size="small" @click="performDelete">Delete</VBtn>
        </VCardActions>
      </VCard>
    </VDialog>

    <!-- Snackbar -->
    <VSnackbar v-model="snackbar.open" :timeout="snackbar.timeout" :color="snackbar.color" location="top-right">
      <div class="d-flex align-center gap-3">
        <VIcon v-if="snackbar.color === 'success'" icon="mdi-check-circle" />
        <VIcon v-else icon="mdi-alert-circle" />
